...
def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=True, exclude=True, cleanup=True):
    """
    Download and prepare the QM9 (GDB9) dataset, checking if files already exist locally.
    """
    ...
    logging.info(
        'Preparing GDB9 dataset. Output will be in directory: {}.'.format(gdb9dir))
    ...
    if not exists(gdb9_tar_data):
        logging.info('File not found locally. Beginning download of GDB9 dataset!')
        urllib.request.urlretrieve(gdb9_url_data, filename=gdb9_tar_data)
        logging.info('GDB9 dataset downloaded successfully!')
    else:
        logging.info('GDB9 dataset already exists locally. Skipping download.')
...
def gen_splits_gdb9(gdb9dir, cleanup=True):
    ...
    if not exists(gdb9_txt_excluded):
        logging.info('File not found locally. Beginning download of GDB9 dataset split!')
        urllib.request.urlretrieve(gdb9_url_excluded, filename=gdb9_txt_excluded)
        logging.info('GDB9 dataset split downloaded successfully!')
    else:
        logging.info('GDB9 dataset split already exists locally. Skipping download.')
...
def get_thermo_dict(gdb9dir, cleanup=True):
    ...
    if not exists(gdb9_txt_thermo):
        logging.info('File not found locally. Beginning download of atomref.txt!')
        urllib.request.urlretrieve(gdb9_url_thermo, filename=gdb9_txt_thermo)
        logging.info('atomref.txt split downloaded successfully!')
    else:
        logging.info('atomref.txtsplit already exists locally. Skipping download.')
